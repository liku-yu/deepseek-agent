...
```bash
./build.sh              # -> C:/develop/bin/deepseek-agent.exe(OUT=dist 可改输出目录)
```

`build.sh` 先 `uv sync` 装依赖,再用 `pyinstaller` 打成单文件 exe。等价的直接命令:
```bash
...
`agent.py` 核心逻辑 · `tui.py` 界面 · `build.sh` 构建 exe · `deepseek-agent.spec` 打包配置 · `pyproject.toml`/`uv.lock` 依赖 · `.env.example` key 模板
